(function () {
    ...
    */
  const isOperatorValid = (par)=>{
      return (par==='-' || par=== '+' || par==='*' || par==='/' || par === '%' )
  }
  console.log(isOperatorValid('/'))
    ...
    */
    function calculator(paran){
        if(isOperatorValid(paran)){
            return function(a,b){
                if(typeof a == 'number' && typeof b == 'number'){
                    return operation[paran](a,b)
                }
    ...
    */
   const showOperetionMessage = (ope, num1, num2) =>{
       return `A operação ${num1} ${ope} ${num2} = `
   }
    ...
    */
    const showErrorMessage = (pan)=>{
        if(!isOperatorValid(pan)){
            `Operação ${pan} não permitida`
        }
    }

(function (win, doc) {
    ...
    */
    $button.addEventListener('click', (event)=>{
        ...
        $inputUsername.value === 'Desconhecido' || $inputUsername.value ==='' ?  alert(
            "Preencha o nome do usuário!"
        ) : true;
        $inputEmail.value === '' ? alert( "Preencha o e-mail!") : true;
        $message.value === '' ? alert('Preencha a mensagem!') : true;
    ...
    */
    const isValidEmail = (email)=>{
        let emailverificar;
        email.replace(/[\w+.]+@\w+(.\w\w)?/, emailverificar);
        if(emailverifica!=email){
